...
import {DataModelClass} from './data-model.class';
...
export class StorageDataComponent implements OnInit {
  ...
  public model = new DataModelClass('', '');
  ...
  async storage() {
      ...
      if (this.model.password === '' || this.model.data === '' ) {
      ...
      const data = stringToBytes(this.model.data);
      api.default.switchRootUrl = environment.switchRootUrl;
      api.default.utils.runInStorageTransferCall(
        data,
        this.calls,
        (call) => {
